...
test("an emirate with no modelled tariff says so instead of returning an empty plan", () => {
  const result = plan(makeSite({ emirate: "sharjah" }));
  assert.equal(result.planUnavailableReason, "no-tariff");
...
test("a capped site lists raising the approved load as a step with a kWp benefit", () => {
  // 500 kW approved load against a roof that packs ~690 kWp.
  const report = reportFor(makeSite({ approvedLoadKw: 500, annualKwh: 3_000_000 }));
  assert.equal(report.recommendation?.bindingConstraint, "approved-load");
  const raise = report.actions.find((step) => step.action.includes("approved load"));
  assert.ok(raise, "expected an approved-load step");
  assert.ok((raise!.benefitKwp ?? 0) > 0, "the step must quantify what it unlocks");
});

test("a no-tariff emirate report leads with getting the tariff, not a fake plan", () => {
  const report = reportFor(makeSite({ emirate: "sharjah" }));
  assert.equal(report.planUnavailableReason, "no-tariff");
  assert.equal(report.recommendation, null);
  assert.ok(report.actions.length > 0);
  assert.ok(
    report.actions[0].action.toLowerCase().includes("tariff"),
    "the first step must be getting a price for a kWh",
  );
});
