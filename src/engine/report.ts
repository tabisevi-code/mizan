...
export const buildReport = (input: ReportInput): SiteReport => {
  ...
  if (result.planUnavailableReason === "no-tariff") {
    ...
    if (
      (best.bindingConstraint === "approved-load" ||
        best.bindingConstraint === "tcl-slab" ||
        best.bindingConstraint === "plot-cap") &&
      cappedKwp > 1
    ) {
      actions.push({
        order: actions.length + 1,
        action:
          best.bindingConstraint === "tcl-slab"
            ? "Apply to raise the total connected load / connection capacity"
            : "Apply to raise the approved load / connection capacity",
