...
import { RULE_SETS, regulatoryCapFor } from "./rules";
import { TURBINES, windClimate, windYield, type Turbine } from "./wind";
import { nearestWindSite } from "./wind-sites";
...
export const recommendMix = (input: RecommendInput, hurdleYears = 8): Recommendation => {
  ...
  if (input.approvedLoadKw) {
    const cap = regulatoryCapFor(input.emirate, input.approvedLoadKw);
    if (cap.capKw < solarCapKw) solarCapKw = cap.capKw;
    solarCapNote = cap.explanation;
  }
